...
```
/consolidate → /design → /plan → /implement -> /ci
...
- Updates `.cursor/memory/` files
- Documents patterns, decisions, and conventions

**Memory files:**
- `.cursor/memory/constitution.md` - Core principles
- `.cursor/memory/architecture.md` - System architecture
- `.cursor/memory/api-patterns.md` - API design patterns
- `.cursor/memory/database-patterns.md` - DB patterns
- `.cursor/memory/testing-patterns.md` - Testing approaches
- `.cursor/memory/integration-patterns.md` - External integrations
...
**What it does:**
- Creates `.cursor/scopes/{date-prefix}-FeatureName/design.md` (date format: `YYYYMMDD-HHMMSS`)
...
**What it does:**
- **Detects active scope**: Identifies feature from `.cursor/scopes/{date-prefix}-FeatureName/`
...
- Creates `.cursor/ci-results.md` with detailed results
...
**Output:**
- `.cursor/ci-results.md` with check status, fixes applied, remaining issues
...
- **Scope location**: `.cursor/scopes/{date-prefix}-FeatureName/` (e.g., `.cursor/scopes/20251121-123456-FeatureName/`)
...
- Pattern updates in `.cursor/memory/` files
