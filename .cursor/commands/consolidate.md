...
1. **Detect active scope** (if applicable):
   - Check for active feature scope: Look for most recent `.cursor/scopes/*/design.md` or `tasks.md`
...
3. **Check memory documentation first**:
   - Read `.cursor/memory/constitution.md` (always)
   - Check for existing memory files in `.cursor/memory/` related to the topic
...
5. **Update memory documentation**:
   ...
   - Create or update memory files in `.cursor/memory/`
   - Document:
...
- `.cursor/memory/constitution.md` - Core principles (already exists)
- `.cursor/memory/architecture.md` - System architecture patterns
- `.cursor/memory/api-patterns.md` - API design patterns
- `.cursor/memory/database-patterns.md` - Database and migration patterns
- `.cursor/memory/testing-patterns.md` - Testing approaches
- `.cursor/memory/integration-patterns.md` - External integrations (FormX, etc.)
