...
1. **Detect active scope and feature directory**:
   ...
   - If not, look for most recent `.cursor/scopes/*/design.md`
   ...
   - For each user scenario from design.md:
     ...
     - **Capture visual evidence**:
       - Take screenshot using `browser_take_screenshot`:
         - **Use relative path** (not absolute): `.cursor/scopes/{FeatureName}/screenshots/{scenario-name}-{timestamp}.png`
         - Example call: `browser_take_screenshot({ fullPage: true, filename: ".cursor/scopes/TestCommandFeedbackLoop/screenshots/scenario-1-happy-path-initial-20251121.png" })`
         - **Do NOT use**: Absolute paths like `/Users/.../formx-sino/.cursor/scopes/...` or variables like `FEATURE_DIR/screenshots/...`
       ...
       - **Retrieve screenshot from temp directory** (CRITICAL - do immediately):
         ...
         - **Solution**: Try multiple retrieval methods, document if fails:
           ...
           SCREENSHOT_PATH=".cursor/scopes/{FeatureName}/screenshots/$SCREENSHOT_FILENAME"
           TEMP_PATH="/var/folders/.../cursor-browser-extension/.../.cursor/scopes/{FeatureName}/screenshots/$SCREENSHOT_FILENAME"
       ...
       - Get accessibility snapshot using `browser_snapshot`:
         ...
         - Save snapshot to `.cursor/scopes/{FeatureName}/screenshots/{scenario-name}-{timestamp}-snapshot.json`
...
- Format: `.cursor/scopes/{FeatureName}/screenshots/{scenario-name}-{timestamp}.png`
- Example call: `browser_take_screenshot({ fullPage: true, filename: ".cursor/scopes/TestCommandFeedbackLoop/screenshots/scenario-1-happy-path-initial-20251121.png" })`
- **Do NOT use absolute paths** like `/Users/.../formx-sino/.cursor/scopes/...` or variables like `FEATURE_DIR/screenshots/...`
...
# The tool result shows: "/var/folders/.../cursor-browser-extension/.../.cursor/scopes/{FeatureName}/screenshots/{filename}.png"
TEMP_PATH="/var/folders/.../cursor-browser-extension/.../.cursor/scopes/{FeatureName}/screenshots/{filename}.png"

# Verify file exists before copying
if [ -f "$TEMP_PATH" ]; then
  cp "$TEMP_PATH" ".cursor/scopes/{FeatureName}/screenshots/" && echo "✓ Copied: $(basename "$TEMP_PATH")"
  
  # Verify screenshot exists in feature directory after copying
  FINAL_PATH=".cursor/scopes/{FeatureName}/screenshots/$(basename "$TEMP_PATH")"
...
find /var/folders -path "*cursor-browser-extension*" -path "*scopes/{FeatureName}/screenshots/*.png" -type f 2>/dev/null | \
  while read file; do 
    if [ -f "$file" ]; then
      cp "$file" ".cursor/scopes/{FeatureName}/screenshots/$(basename "$file")"
      FINAL_PATH=".cursor/scopes/{FeatureName}/screenshots/$(basename "$file")"
      if [ -f "$FINAL_PATH" ]; then
...
4. **Take screenshot and snapshot**:
   - Screenshot: `browser_take_screenshot({ fullPage: true, filename: ".cursor/scopes/{FeatureName}/screenshots/scenario-1-happy-path-initial-{timestamp}.png" })`
   ...
   - Snapshot: `browser_snapshot()` → Save to `.cursor/scopes/{FeatureName}/screenshots/scenario-1-happy-path-initial-{timestamp}-snapshot.json`
...
- Example:
  ...
  const screenshotPath = '.cursor/scopes/{FeatureName}/screenshots/{filename}.png';
  ...
  SCREENSHOT_PATH=".cursor/scopes/{FeatureName}/screenshots/{filename}.png"
...
- **Use relative paths from workspace root** when calling `browser_take_screenshot`:
  - Format: `.cursor/scopes/{FeatureName}/screenshots/{filename}.png`
  - Example: `.cursor/scopes/TestCommandFeedbackLoop/screenshots/scenario-1-happy-path-initial-20251121.png`
